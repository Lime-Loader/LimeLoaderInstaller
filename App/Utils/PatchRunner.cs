...
public static class PatchRunner
{
    ...
    private static async Task<bool> DownloadMelonData(string destination, string? customMelonDataPath = null)
    {
        ...
        try
        {
            ...
            string releaseInfo = await client.GetStringAsync("https://api.github.com/repos/Lime-Loader/LimeLoader/releases/latest");
    ...
    private static async Task RestoreAppData(UnityApplicationFinder.Data data)
    {
        ...
        if (Directory.Exists(dataBackupPath) && Directory.GetFileSystemEntries(dataBackupPath).Length > 0)
        {
            ...
            {
                bool res = await PopupHelper.TwoAnswerQuestion("Lime is unable to restore app data, do you want to open the folder containing the back up now? It will not be deleted after patching is complete.", "Unable to Restore", "Yes", "No");
                if (res)
                {
                    Process proc = new()
                    {
